...
var WEBIM={
    ...
    handlerMessageByType:function(message){
		...
		switch (message.type){
	        ...
	        case MessageType.Type.UPDATE_ROOM_INFO: //更新群组信息的群控制消息，web 端不处理
	        	break;
	...
	createCommApiSecret(obj){
		...
		if(!myFn.isNil(obj.userId)&&!myFn.isNil(obj.access_token)){
			key = AppConfig.apiKey+obj.time+obj.userId+obj.access_token;
		}else{
			key = AppConfig.apiKey+obj.time+WEBIM.userId+WEBIM.token;
		}
