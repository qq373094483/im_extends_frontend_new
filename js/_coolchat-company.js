		...
		createCompanyItem :function(obj){   
			...
			var itemHtml = "<div id='company_"+obj.id+"'>"
	                           ...
	                           if (employeeRole[obj.id]>=2)  //权限判断
							   		itemHtml += "<a href='#' style='display:block;height:8px;' onclick='CompanyManager.showCreateDepartment(\"" + obj.id + "\")'>创建部门</a><br/>"
							   				+"<a href='#' style='display:block;height:8px;' onclick='CompanyManager.modifyCompanyInfo(\"" + obj.id + "\")'>修改信息</a><br/>"
							   				+"<a href='#' style='display:block;height:8px;' onclick='CompanyManager.setAdministrator(\"" + obj.id + "\")'>设置管理员</a><br/>";


							   itemHtml += "<a style='display:block;height:8px;' href='#' onclick='CompanyManager.modifyMyPosition(\"" + obj.id + "\")'>我的职位</a><br/>"
							   if(obj.createUserId==myData.userId){
							   		itemHtml+="<a href='#' onclick='CompanyManager.quitCompany(\"" + obj.id + "\")'>解散公司</a>";
							   }else {
							   		itemHtml+="<a href='#' onclick='CompanyManager.quitCompany(\"" + obj.id + "\")'>退出公司</a>";
							   }
							   itemHtml+="</div>"
			...
			for (var i = 0; i < length; i++) {
				...
				if (department.parentId == rootDepartId) {
					...
					var departHtml = "<div id='department_"+department.id+"'> "
							        ...
							        +   "</div>"  
							        +   "<div id='departMenu_"+department.id+"' class='departMenu' style='opacity: 0.8;position: absolute; right: 1%; display: none;' onmouseout='CompanyManager.hideDepartmentMenu(\"" + department.id + "\",event);'>" 

							        if (employeeRole[department.companyId]>=1) //权限判断
							        	departHtml += "<a style='display:block;height:8px;' href='#' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</a><br/>"
												   + "<a style='display:block;height:8px;' href='#' onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</a><br/>"
							        			   + "<a style='display:block;height:8px;' href='#' onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</a><br/>";
